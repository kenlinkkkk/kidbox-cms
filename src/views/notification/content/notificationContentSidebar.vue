  ...
  <vs-sidebar class="add-new-data-sidebar items-no-padding" click-not-close color="primary" default-index="1" parent="body"
    ...
    <component :is="scrollbarTag" :key="$vs.rtl" :settings="settings" class="scroll-area--data-list-add-new">
      ...
      <div class="p-6">
        ...
        <div class="vs-component vs-con-input-label vs-input mt-5 w-full vs-input-primary"
             v-if="this.dataId === null && $acl.check('Master')">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      parents() {
        return this.$store.getters["userManagement/getParents"]
      },
      isFormValid() {
    ...
    methods: {
      ...
      inputClasses() {
        ...
        this.$store.dispatch('userManagement/getUserByClass', {
          "key_word": '',
      ...
      searchUsers(search, loading) {
        loading(true)
        this.$store.dispatch('userManagement/getUserByClass', {"name": search, "class_ids": this.class_ids});
